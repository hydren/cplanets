...
: DialogBgrWin(Rect(0, 0, 400, 120), "Choose file"), mode(mode),
...
void FileDialog::setPosition(Point position)
{
	...
	setComponentPosition(&lblCurrentDirectory, Point(8, titleBarArea.y + titleBarArea.h + 8));
...
void FileDialog::triggerNavigation(Button* navButton)
{
	...
	if(self->mode == SELECT_FILE)
		file_chooser(FileDialog::folderOpened, self->id);
}
