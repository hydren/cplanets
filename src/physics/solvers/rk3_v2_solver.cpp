...
void Rk3V2Solver::step()
{
	...
	foreach(Body2D*, body, vector<Body2D*>, universe.bodies)
	{
		...
		wv = body->velocity + k2v * timestep * (2.0/3.0);
		wr = body->position + k2r * timestep * (2.0/3.0);
	}
	...
	foreach(Body2D*, body, vector<Body2D*>, universe.bodies)
	{
		body->velocity += (k1vs[body] + k3vs[body]*3.0) * timestep * 0.25;
		body->position += (k1rs[body] + k3rs[body]*3.0) * timestep * 0.25;
	}
